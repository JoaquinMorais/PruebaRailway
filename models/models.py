...
class SubDependencia(db.Model):
    __tablename__ = 'subdependencia'
    id_subdependencia = db.Column(db.Integer, primary_key=True)
    id_subpadre = db.Column(db.Integer, ForeignKey('subproyecto.id_subproyecto', ondelete='SET NULL', onupdate='CASCADE'))
    id_subhijo = db.Column(db.Integer,  ForeignKey('subproyecto.id_subproyecto', ondelete='SET NULL', onupdate='CASCADE'))

    def __init__(self, subpadre, subhijo):
        self.id_subpadre = subpadre
        self.id_subhijo = subhijo
...
class AtributoGenerico(db.Model):
    ...
    hijos = db.relationship('AtributoDependiente', lazy = True)
...
class AtributoDependiente(db.Model):
    __tablename__ = 'atributodependiente'
    id_atributodependiente = db.Column(db.Integer, primary_key=True)
    titulo = db.Column(db.String(50), default = None) # unique=True
    valor = db.Column(db.String(1000), default = None)
    id_atributogenerico = db.Column(db.Integer, ForeignKey('atributogenerico.id_atributogenerico', ondelete='SET NULL', onupdate='CASCADE'))

    def __init__(self, titulo, valor, atributopadre):
        self.titulo = titulo
        self.valor = valor
        self.id_atributogenerico = atributopadre
